...
export function PreloaderBlock({ block, onComplete }: PreloaderBlockProps) {
  ...
  const [shouldRenderMask, setShouldRenderMask] = useState(() => {
    if (typeof window !== 'undefined') {
      return sessionStorage.getItem('hc-preloader-seen') !== '1'
    }
    // На сервері ніколи не рендеримо маску, щоб уникнути мікро-появи до гідрації
    return false
  })
  ...
  useGSAP(() => {
    if (!shouldRenderMask) {
      // Пропускаємо анімацію маски у межах поточної сесії і взагалі не рендеримо maskRef
      document.body.style.height = 'auto'
      document.body.style.overflow = 'unset'
      // Але залишаємо приємну появу заголовка/підзаголовка
      gsap.fromTo(
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        ...
        document.body.style.height = 'auto'
        document.body.style.overflow = 'unset'
        sessionStorage.setItem('hc-preloader-seen', '1')
        setShouldRenderMask(false)
      },
    });
  ...
  return (
    <section>
      ...
      >
        ...
        {shouldRenderMask && (
          <div className="absolute inset-0 z-10 pointer-events-none will-change-transform [contain:paint]" ref={maskRef}>
